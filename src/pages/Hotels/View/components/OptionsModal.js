...
class OptionsModal extends Component {
  ...
  handleTakeOption = () => {
    ...
    this.props.createBooking(provisionalBookingValues)
      .then(() => { this.props.history.push(`/holds/`) }
      );
  };
    ...
    return (
      ...
      >
        <Container>
          ...
          <Row>
            <Note>
                <Text>Will hold the room(s) on your behalf for 24 hours.  Unless you confirm the room in writing, by <strong>{timeUntilOptionRelease()}</strong> it will automatically released.</Text>
            </Note>
          </Row>
          <Row>
            <MainButton onClick={this.handleTakeOption}>
              <MainText>
                TAKE AN OPTION ON THIS BOOKING
              </MainText>
            </MainButton>
          </Row>
        </Container>
...
export default withRouter(connect(undefined, { createBooking })(OptionsModal));
