...
describe('Offer Edit Applicaions: Transfer Discount', () => {
  const fieldsetClass = '.transferDiscountFieldset';
  const discountClass = '.transferDiscountGrid';
  const fieldName = 'transferDiscounts';
  let transferDiscountProps;
  let transferDiscountSubject;

  const transferDiscounts: IUIOfferProductDiscountInstance[] = [
    {
      uuid: 'TRANSFER_DISCOUNT_UUID_1',
      discountPercentage: 2.45,
      products: [],
      productCategory: EProductCategory.PER_BOOKING,
    },
    {
      uuid: 'TRANSFER_DISCOUNT_UUID_2',
      discountPercentage: 2.57,
      productCategory: EProductCategory.PER_BOOKING,
      products: [],
    },
    {
      uuid: 'TRANSFER_DISCOUNT_UUID_3',
      discountPercentage: 3.78,
      productCategory: EProductCategory.PER_BOOKING,
      products: [],
    },
    {
      uuid: 'TRANSFER_DISCOUNT_UUID_PER_PESON_1',
    ...
    {
      uuid: 'TRANSFER_DISCOUNT_UUID_PER_PESON_2',
      discountPercentage: 4.44,
      maximumQuantity: 5,
  ...
  const availableTransferProducts = makeAvailableProducts('transfer', [
  ...
  beforeEach(() => {
    ...
    transferDiscountProps = props;
    transferDiscountSubject = subject;
  });
  ...
  it('renders multiple transferDiscounts', () => {
    expect(transferDiscountSubject.find(discountClass).length).toBe(transferDiscountProps[fieldName].length);
  });

  it('renders the correct product category', () => {
    const labels = transferDiscountSubject.at(0).find('.category');
    expect(
      labels
        .at(0)
        .props()
  ...
  it('handles the remove discount event correctly', () => {
    transferDiscountSubject
      .find(discountClass)
      .at(0)
      .find(CloseButton)
      .simulate('click');
    expect(transferDiscountProps.offerRemoveProductDiscountAction).toHaveBeenCalledTimes(1);
    expect(transferDiscountProps.offerRemoveProductDiscountAction).toHaveBeenCalledWith(
      'Transfer',
      'TRANSFER_DISCOUNT_UUID_1'
    );
  });

  it('handles the add discount event correctly', () => {
    transferDiscountSubject.find('.addDiscount').simulate('click');

    expect(transferDiscountProps.offerAddProductDiscountAction).toHaveBeenCalledTimes(1);
    expect(transferDiscountProps.offerAddProductDiscountAction).toHaveBeenCalledWith('Transfer');
  });
  ...
  describe('Discount Percentage', () => {
    it('Renders the correct value for the first discount', () => {
      const inputValue = transferDiscountSubject
        .find(discountClass)
        .at(0)
        .find('.discountInput')
        .find(TextInput)
      ...
      expect(inputValue).toBe(transferDiscounts[0].discountPercentage);
    });
    ...
    it('renders the correct value for the second discount', () => {
      ...
      expect(inputValue).toBe(transferDiscounts[1].discountPercentage);
    });
    ...
    it('handles the change event for the first discount', () => {
      ...
      expect(transferDiscountProps.offerUpdateProductDiscountAction).toHaveBeenCalledTimes(1);
      expect(transferDiscountProps.offerUpdateProductDiscountAction).toHaveBeenCalledWith(
        'Transfer',
        'TRANSFER_DISCOUNT_UUID_1',
        'discountPercentage',
        '999'
  ...
  describe('Maximum Quanity', () => {
    it('Does not render a maximum quantity input for perBooking products', () => {
      const maxQuantity = transferDiscountSubject
        .find(discountClass)
        .at(0)
    ...
    it('Renders the correct value for the first discount', () => {
      ...
      expect(inputValue).toBe(transferDiscounts[3].maximumQuantity);
    });
    ...
    it('handles the change event for the first discount', () => {
      ...
      expect(transferDiscountProps.offerUpdateProductDiscountAction).toHaveBeenCalledWith(
        'Transfer',
        'TRANSFER_DISCOUNT_UUID_PER_PESON_1',
        'maximumQuantity',
        '999'
      );
    });
  ...
  describe('Standard Occupancy', () => {
    ...
    it('renders the correct value for the fourth discount', () => {
      ...
      expect(inputValue).toBe(transferDiscounts[3].standardOccupancyOnly);
    });
    ...
    it('renders the correct value for the fifth discount', () => {
      ...
      expect(inputValue).toBe(transferDiscounts[4].standardOccupancyOnly);
    });
    ...
    it('handles the change event correctly', () => {
      ...
      expect(transferDiscountProps.offerUpdateProductDiscountAction).toHaveBeenCalledWith(
        'Transfer',
        'TRANSFER_DISCOUNT_UUID_PER_PESON_2',
  ...
  describe('Age names', () => {
    ...
    const discountsWithProducts: IUIOfferProductDiscountInstance[] = [
      {
        uuid: 'TRANSFER_DISCOUNT_UUID_1',
        discountPercentage: 2.45,
        products: [{ uuid: 'TRANSFER_PRODUCT_0', ageNames: ['Child', 'Infant'] }],
        productCategory: EProductCategory.PER_PERSON,
      },
      {
        uuid: 'TRANSFER_DISCOUNT_UUID_2',
    ...
    const availableTransferProducts = makeAvailableProducts('Transfer', [EProductCategory.PER_PERSON], ageNames);

    it('Renders the age names accordian correctly', () => {
      const { subject } = setupTest(fieldsetClass, {
        transferDiscounts: discountsWithProducts,
        availableTransferProducts,
      });
      ...
      const discounts: IUIOfferProductDiscountInstance[] = [
        {
          ...
          discountPercentage: 2.45,
          //@ts-ignore
          products: [{ uuid: 'TRANSFER_PRODUCT_0', ageNames: undefined }],
          productCategory: EProductCategory.PER_PERSON,
        },
      ];

      const { subject } = setupTest(fieldsetClass, {
        transferDiscounts: discounts,
        availableTransferProducts,
      });
